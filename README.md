...
- **Chrome themes** - The overflow menu's **Theme** row switches the editor between Paper (the default, warm light), Brass (the original Lavish dark look), Daylight (cool light), Graphite (neutral dark), and Fjord (deep navy). The choice is remembered per browser and applies to every review tab on its next load. A theme restyles only Lavish's own UI - the bar, the Conversation panel, menus, and the annotation card and outline inside the artifact frame - never the artifact itself. While Lavish is serving it, the artifact's `<html>` carries the choice as `data-lavish-theme` (`brass`, `paper`, `daylight`, `graphite`, or `fjord`), so an artifact written to follow the editor can key its palettes off that attribute; opened directly, the attribute is absent and the artifact shows its own default. `lavish-axi design` points agents at `src/design/lavish-themes.css`: DaisyUI themes for the same five palettes (`data-theme="lavish"` follows the editor, `lavish-paper` and friends are fixed), with `primary` kept for what needs your decision and `<mark>` drawn as a highlighter.
